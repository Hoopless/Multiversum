...
const ProductShow: FC<{id: number | string}> = ({id}) => {
	const productInfo: ConsumerProduct = useSWR(`/product?id=${id}`, swrFetcherJSON, {
		loadingTimeout: 0,
	...
	return  (
		<>
			...
			>
				...
				<Box pl={['0px', '0px', '3rem']} width={['100%', 8 / 12, 8 / 12, 8 / 12]}>
					<Flex>
						<Box w='50%'>
							<Text fontSize='xl' mb='30px'>{productInfo.name}</Text>
						</Box>
						<Flex w='50%'>
							<Box mx='10px'>
					...
					<Flex justifyContent='end' >
						<Text ml='auto' fontSize='xl' mr='6rem'>€ {currencyFormat(productInfo.price)}</Text>
					</Flex>
